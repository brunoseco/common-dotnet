    ...
    internal class Banco_Bradesco : AbstractBanco, IBanco
    {
        ...
        private string GerarHeaderLoteRemessaCNAB240(Cedente cedente, int numeroArquivoRemessa)
        {
            try
            {
                ...
                var dvConta = cedente.ContaBancaria.DigitoConta ?? "0";
                ...
                header += Utils.FormatCode(numeroArquivoRemessa.ToString(), "0", 8, true);        //184-191  8 Número de Remessa
                header += DateTime.Now.ToString("ddMMyyyy");                                      //192-199  6 Data de gravação da remessa  DDMMAAAA 
                header += Utils.FormatCode("0", "0", 8);                                          //200-207  8 Zeros
        ...
        public string GerarHeaderRemessaCNAB240(Cedente cedente, int numeroArquivoRemessa)
        {
            try
            {
                ...
                header += Utils.FormatCode(numeroArquivoRemessa.ToString(), "0", 6, true);      //Nº sequencial do arquivo                  158-163
        ...
        public override string GerarDetalheSegmentoPRemessa(Boleto boleto, int numeroRegistro, string codigoTransmissao)
        {
            try
            {
                ...
                _dacNossoNumero = this.CalcularDigitoNossoNumero(boleto);
                boleto.DigitoNossoNumero = _dacNossoNumero;

                //Identificação do titulo consta Codigo da Carteira + Zeros + Nosso Numero + DV Nosso Numero
                var identificacaoTitulo = carteira + Utils.FitStringLength(boleto.NossoNumero + boleto.DigitoNossoNumero, 17, 17, '0', 0, true, true, true);
                #endregion
                ...
                if (boleto.PercJurosMora > 0)
                {
                    ...
                    _segmentoP += Utils.FitStringLength(boleto.JurosMora.ToString("0.00").Replace(",", ""), 15, 15, '0', 0, true, true, true);     //127-141
                }
                ...
                _segmentoP = this.InsereCodigoParaBaixaDevolucao(_segmentoP);                                                                                                          //224-224

                _segmentoP += "000090000000000 ";                                                                                           //225-240
        ...
        private string InsereCodigoParaBaixaDevolucao(string _segmentoP, string codigo = "1")
        {
            //1 = Baixar/Devolver
            _segmentoP += codigo;
            return _segmentoP;
        ...
        public override string GerarDetalheSegmentoQRemessa(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo)
        {
            try
            {
                ...
                #region CPF/CNPJ Sacado
                var cpfcnpjFormatado = string.Empty;
                if (boleto.Sacado.CPFCNPJ.Length <= 11)
                {
                    _segmentoQ += "1";

                    cpfcnpjFormatado = Utils.FormatCode(boleto.Sacado.CPFCNPJ, "0", 11, true);
                    cpfcnpjFormatado = cpfcnpjFormatado.Substring(0, 9) + "0000" + cpfcnpjFormatado.Substring(9, 2);
                }
                else
                {
                    _segmentoQ += "2";

                    cpfcnpjFormatado = boleto.Sacado.CPFCNPJ;
                }

                _segmentoQ += Utils.FitStringLength(cpfcnpjFormatado, 15, 15, '0', 0, true, true, true);
                ...
                _segmentoQ += "000";
                _segmentoQ += "00000000000000000000";
                _segmentoQ += Utils.FitStringLength("", 8, 8, ' ', 0, true, true, true);
