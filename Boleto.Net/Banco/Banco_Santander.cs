...
namespace BoletoNet
{
    internal class Banco_Santander : AbstractBanco, IBanco
    {
        ...
        private void ValidaNossoNumero(Boleto boleto)
        {
            int nossonumero;

            Int32.TryParse(boleto.NossoNumero, out nossonumero);
            if (nossonumero.ToString().Length > 7)
                throw new NotSupportedException("Santander - Nosso Número deve ter no máximo 7 dígitos.");
        }
        ...
        public override void ValidaBoleto(Boleto boleto)
        {
            ...
            if (this.Codigo == 8)
            {
                if (boleto.NossoNumero.Length != 8)
                    throw new NotImplementedException("Santander - Nosso Número deve ter 8 posições para o banco 008.");
            }
        ...
        public string GerarHeaderRemessaCNAB240(Cedente cedente, int numeroArquivoRemessa)
        {
            try
            {
                ...
                header += Utils.FormatCode(codigoTransmissao, "0", 15, true);                       //Codigo transmissão adquirido pelo banco   033-047 
        ...
        private string GerarHeaderLoteRemessaCNAB240(Cedente cedente, int numeroArquivoRemessa)
        {
            try
            {
                ...
                header += Utils.FormatCode(codigoTransmissao, "0", 15, true);                       //Código de transmissão adquirido pelo banco    054-068     
