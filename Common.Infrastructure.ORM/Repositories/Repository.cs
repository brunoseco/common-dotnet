...
namespace Common.Infrastructure.ORM.Repositories
{
    public class Repository<T, U> : RepositoryBase<U>, IRepository<T> where T : class
    {
        ...
        public IQueryable<T> GetAllAsTracking(string path)
        {
            return this.ctx.Set<T>().Include(path);
        }
        public IQueryable<T> GetAllAsTracking(params Expression<Func<T, object>>[] includes)
        {
        ...
        public T Update(T modelNew)
        {
            var entry = this.ctx.Entry(modelNew);
            this.dbSet.Attach(modelNew);
        ...
        private bool ExistsInContext(T entity)
        {
            return this.ctx.Set<T>().Local.Any(e => e == entity);
        }
