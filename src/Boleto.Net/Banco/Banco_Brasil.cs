    ...
    internal class Banco_Brasil : AbstractBanco, IBanco
    {
        ...
        public override void ValidaBoleto(Boleto boleto)
        {
            ...
            if (!boleto.Carteira.Equals("11") &
                ...
                throw new NotImplementedException("Banco do Brasil - Carteira não informada ou incorreta. Utilize a carteira 11, 16, 17, 17-019, 17-035, 17-051, 17-027, 18, 18-019, 18-027, 18-035, 18-140 ou 31.");
        ...
        public override void FormataNossoNumero(Boleto boleto)
        {
            ...
            else if (boleto.Cedente.Convenio.ToString().Length == 6)
            {
                if (!boleto.Carteira.Equals("16"))
                {
                    if (this.NossoNumeroCarteiraDezoitoJaFormatado(boleto))
                        return;
            ...
            switch (boleto.Carteira)
            {
                ...
                case "16":
                case "18":
                    boleto.NossoNumero = string.Format("{0}", Utils.FormatCode(boleto.NossoNumero, "0", 11, true));
                    return;
            }
        ...
        private bool NossoNumeroCarteiraDezoitoJaFormatado(Boleto boleto)
        {
            /*Esta carteira pede 11 dígitos no nosso número, mas, devido à formatação,
             passa a ter 13 por conta dos caracteres '-1' no final. Se estiver neste formato, já foi formatado.*/
            return boleto.Carteira.Equals("18") && boleto.NossoNumero.Length == 13;
        }
        ...
        public override DetalheRetorno LerDetalheRetorno7CNAB400(string registro)
        {
            try
            {
                ...
                decimal tarifaCobranca = Convert.ToInt64(registro.Substring(181, 7));
                detalhe.TarifaCobranca = tarifaCobranca / 100;
        ...
        public override string GerarDetalheSegmentoPRemessa(Boleto boleto, int numeroRegistro, string numeroConvenio)
        {
            try
            {
                ...
                if (!boleto.Cedente.Convenio.ToString().Length.Equals(7))
                    _nossoNumero = boleto.NossoNumero.Replace("-", "").Replace("/", "");
                else
                    _nossoNumero = boleto.NossoNumero;
                #endregion
                ...
                if (boleto.Carteira.Equals("17-019") ||
                    ...
                    boleto.Carteira.Equals("17-035") ||
                    boleto.Carteira.Equals("17") ||
                    boleto.Carteira.Equals("11") ||
                    boleto.Carteira.Equals("12"))
                    _segmentoP += "1";
                else
                    _segmentoP += "0";
                #endregion
        ...
        public override HeaderRetorno LerHeaderRetornoCNAB240(string registro)
        {
            try
            {
                HeaderRetorno header = new HeaderRetorno();
                header.RegistroHeader = registro;
                header.Agencia = Convert.ToInt32(registro.Substring(52, 5).Replace("X", "0"));
                header.DigitoAgencia = Convert.ToInt32(registro.Substring(57, 1).Replace("X", "0"));
                header.ContaCorrente = Convert.ToInt32(registro.Substring(58, 12).Replace("X", "0"));
                header.DigitoContaCorrente = Convert.ToInt32(registro.Substring(70, 1).Replace("X", "0"));
                header.CodigoCedente = Convert.ToInt32(registro.Substring(32, 9).Replace("X", "0"));
        ...
        public override DetalheSegmentoTRetornoCNAB240 LerDetalheSegmentoTRetornoCNAB240(string registro)
        {
            try
            {
                ...
                if (Int32.TryParse(registro.Substring(44, 12), out nossoNumero))
                    segmentoT.NossoNumero = registro.Substring(44, 12);
                else
                    segmentoT.NossoNumero = registro.Substring(58, 15);
