...
namespace BoletoNet
{
    ...
    internal class Banco_Caixa : AbstractBanco, IBanco
    {
        ...
        public override void FormataNossoNumero(Boleto boleto)
        {
            ...
            if (boleto.Carteira.Equals("RG"))
                boleto.NossoNumero = boleto.NossoNumero + "-" + Mod11Base9(boleto.NossoNumero);
            else
        ...
        public override void ValidaBoleto(Boleto boleto)
        {
            ...
            else
            {
                ...
                {
                    throw new Exception("Nosso Número inválido, Para Caixa Econômica carteira indefinida, o Nosso Número deve conter 8 caracteres.");
                }
            }
        ...
        public string GerarDetalheSegmentoPRemessaCNAB240(Boleto boleto, int numeroRegistro, string numeroConvenio, Cedente cedente)
        {
            try
            {
                this.ValidaInstrucoes240(boleto); // Para protestar, devolver ou desconto.
                ...
                if (valorJuros > 0)
                {
                    ...
                    _segmentoP += Utils.FitStringLength(valorJuros.ToString("0.00").Replace(",", ""), 15, 15, '0', 0, true, true, true); // Juros de Mora por Dia/Taxa 
                }
                else
                {
                ...
                if (boleto.ValorDesconto > 0)
                {
                    ...
                    _segmentoP += Utils.FitStringLength(boleto.ValorDesconto.ToString("0.00").Replace(",", ""), 15, 15, '0', 0, true, true, true);  // Valor/Percentual a ser Concedido 
                }
                else
                {
                ...
                _segmentoP += Utils.FitStringLength(boleto.IOF.ToString("0.00").Replace(",", ""), 15, 15, '0', 0, true, true, true);        // Valor do IOF a ser Recolhido 
                _segmentoP += Utils.FitStringLength(boleto.Abatimento.ToString("0.00").Replace(",", ""), 15, 15, '0', 0, true, true, true); // Valor do Abatimento 
        ...
        public string GerarDetalheSegmentoRRemessaCNAB240(Boleto boleto, int numeroRegistroDetalhe, TipoArquivo CNAB240)
        {
            try
            {
                ...
                _segmentoR += Utils.FormatCode("", "0", 48);                                                // Uso Exclusivo FEBRABAN/CNAB 
                #region Multa
        ...
        public string GerarTrailerLoteRemessaCNAB240(int numeroRegistro, Boleto boletos)
        {
            try
            {
                ...
                _trailerLote += Utils.FormatCode(numeroRegistro.ToString(), "0", 6, true);                    // Nº Sequencial do Registro no Lote 
        ...
        public override DetalheSegmentoTRetornoCNAB240 LerDetalheSegmentoTRetornoCNAB240(string registro)
        {
            try
            {
                ...
                {
                    if (registro.Substring(38, 1) == "0")
                        segmentoT.NossoNumero = registro.Substring(42, 14);
                    else
        ...
        public string GerarHeaderRemessaCNAB400(int numeroConvenio, Cedente cedente, int numeroArquivoRemessa)
        {
            try
            {
                ...
                reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0390, 005, 0, this.RetornaNumeroSequencialRemessaCNAB400(numeroArquivoRemessa, 5), ' '));       //390-394
        ...
        private string RetornaNumeroSequencialRemessaCNAB400(int numeroDaRemessa, int tamanhoDoCampo)
        {
            var quantidadeCaracteresNaRemessa = numeroDaRemessa.ToString().Length;
            StringBuilder numeroSequencial = new StringBuilder();

            while (quantidadeCaracteresNaRemessa < tamanhoDoCampo)
            {
                numeroSequencial.Append("0");
                quantidadeCaracteresNaRemessa++;
            }

            return numeroSequencial.Append(numeroDaRemessa).ToString();
        }
        ...
        public string GerarDetalheRemessaCNAB400(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo)
        {
            try
            {
                ...
                boleto.Moeda = 1; //De acordo com o especificado no manual de cobrança CNAB400, o padrão utilizado para moeda é 1 = REAL
