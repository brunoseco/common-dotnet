...
namespace BoletoNet
{
    ...
    internal class Banco_Itau : AbstractBanco, IBanco
    {
        ...
        public override void ValidaBoleto(Boleto boleto)
        {
            ...
            {
                //Carteiras válidas
                int[] cv = new int[] { 175, 176, 178, 109, 112, 198, 107, 122, 142, 143, 196, 126, 131, 146, 150, 169, 121 };
                bool valida = false;
                ...
                boleto.LocalPagamento = " Até o vencimento, preferencialmente no Itaú e após o vencimento, somente no Itaú";
        ...
        public override void FormataCodigoBarra(Boleto boleto)
        {
            try
            {
                ...
                if (boleto.Carteira == "175" || boleto.Carteira == "176" || boleto.Carteira == "178" || boleto.Carteira == "109" || boleto.Carteira == "112" || boleto.Carteira == "121")//Flavio(fhlviana@hotmail.com) - Adicionado carteira 109
                {
        ...
        public string GerarDetalheRemessaCNAB400(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo)
        {
            try
            {
                ...
                if (boleto.Carteira == "112")
                {
                    _detalhe += Utils.FitStringLength(boleto.NossoNumero, 25, 25, ' ', 0, true, true, false);
                    _detalhe += Utils.FitStringLength("0", 8, 8, '0', 0, true, true, true);
                }
                else
                {
                    _detalhe += nrDocumento;
        ...
        public string GerarDetalheRemessaTipo5CNAB400(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo)
        {
            ...
            if (boleto.ValorMulta > 0)
            {
                ...
                _detalhe += Utils.FitStringLength(boleto.ValorMulta.ToString("0.00").Replace(",", ""), 13, 13, '0', 0, true, true, true);
            }
            else if (boleto.PercMulta > 0)
            {
                _detalhe += "2";
                _detalhe += boleto.DataVencimento.ToString("ddMMyyyy");
                _detalhe += Utils.FitStringLength(boleto.PercMulta.ToString("0.00").Replace(",", ""), 13, 13, '0', 0, true, true, true);
            }
            else
            {
                _detalhe += "0";
                _detalhe += "00000000";
                _detalhe += Utils.FormatCode("0", 13);
            }

            _detalhe += Utils.FormatCode("", " ", 371);
        ...
        public override DetalheRetorno LerDetalheRetornoCNAB400(string registro)
        {
            try
            {
                ...
                var temUsoEmpresa = Int64.TryParse(detalhe.UsoEmpresa, out _usoEmpresa);

                if (registro.Substring(82, 3) == "112" && temUsoEmpresa)
                    detalhe.NossoNumero = _usoEmpresa.ToString();

                else
                {
                    detalhe.NossoNumeroComDV = registro.Substring(85, 9);
                    detalhe.NossoNumero = registro.Substring(85, 8);
                    detalhe.DACNossoNumero = registro.Substring(93, 1);
                }
