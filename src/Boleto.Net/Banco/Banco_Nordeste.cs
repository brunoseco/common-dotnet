    ...
    internal class Banco_Nordeste : AbstractBanco, IBanco
    {
        ...
        public string GerarHeaderRemessaCNAB400(Cedente cedente, int numeroArquivoRemessa)
        {
            try
            {
                ...
                reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0104, 291, 0, string.Empty, ' '));                          //108-129
        ...
        public string GerarDetalheRemessaCNAB400(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo)
        {
            try
            {
                ...
                #region Carteira
                reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0108, 001, 0, boleto.Carteira[1].ToString(), '0'));                           //108-108
                #endregion

                #region Cod Ocorrencia
                ...
                {
                    var datadesc = boleto.DataDesconto.ToString("ddMMyy");
                    var valordesc = boleto.ValorDesconto.ToString("0.00").Replace(",", "");
                    reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediNumericoSemSeparador_, 0174, 006, 0, datadesc, '0'));                              //174-179
                    reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediNumericoSemSeparador_, 0180, 013, 2, valordesc, '0'));                             //180-192
                }
                else
                {
        ...
        public override DetalheRetorno LerDetalheRetornoCNAB400(string registro)
        {
            try
            {
                ...
                {
                    case (int)ECodigoOcorrenciaNordeste400.Entrada_Confirmada:
                    case (int)ECodigoOcorrenciaNordeste400.Abatimento_Concedido:
                    case (int)ECodigoOcorrenciaNordeste400.Abatimento_Cancelado:
                    case (int)ECodigoOcorrenciaNordeste400.Vencimento_Alterado:
                    case (int)ECodigoOcorrenciaNordeste400.Alteração_Depositária:
                    case (int)ECodigoOcorrenciaNordeste400.Confirmação_Protesto:
                    case (int)ECodigoOcorrenciaNordeste400.Confirmação_SustarProtesto:
                    case (int)ECodigoOcorrenciaNordeste400.Alteração_informações_de_Controle_da_Empresa:
                    case (int)ECodigoOcorrenciaNordeste400.Alteração_Seu_Número:
                    case (int)ECodigoOcorrenciaNordeste400.Alteração:
                        detalhe.Aceito = true;
                        break;
                    case (int)ECodigoOcorrenciaNordeste400.Liquidação_Normal:
                    case (int)ECodigoOcorrenciaNordeste400.Pagamento_por_Conta:
                    case (int)ECodigoOcorrenciaNordeste400.Pagamento_por_Cartório:
                    case (int)ECodigoOcorrenciaNordeste400.Baixa_Automática:
                    case (int)ECodigoOcorrenciaNordeste400.Baixa_Simples:
                        detalhe.Baixado = true;
                        detalhe.Aceito = false;
                        break;
                    case (int)ECodigoOcorrenciaNordeste400.Devolvido_Protestado:
                        detalhe.Cancelado = true;
                        detalhe.Aceito = false;
                        break;
                    case (int)ECodigoOcorrenciaNordeste400.Entrada_Rejeitada:
                        detalhe.Aceito = false;
                        break;
                }
